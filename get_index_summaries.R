...
xo     <- readRDS('./screening_value_analysis/rds/kdas_same_day_sampling 2024-03-26.RDS')
...
region <- c(rep('primary',4), rep('nitrogen_region',2),'primary', 'phosphorus_region', 'primary')
...
for(i in 1:nrow(key)){
    ...
    og_sv <- repo$lookups$sv
    colnames(og_sv)[1:4] <- c('primary', 'ECOREGION', 'CHARACTERISTIC','og_value')


    og_sv <- og_sv %>%
             select(primary:og_value) %>%
             mutate(primary = ifelse(str_detect(primary, pattern = 'MISS'), 'MVIR', primary)) %>%
    ...
    if(!character_id %in% og_sv$CHARACTERISTIC){
      og_sv <- og_sv %>% 
        select(matches(region_def)) %>% 
        distinct() %>% 
        mutate(old_sv = 0)
    }
    if(character_id %in% og_sv$CHARACTERISTIC){
      ...
      og_sv <- og_sv %>%
            filter(CHARACTERISTIC %in% character_id)%>%
            mutate(CHARACTERISTIC = 'old_sv') %>% 
            spread(CHARACTERISTIC, value = og_value) %>%
            select(matches(region_def), old_sv) %>%
            distinct()
    }
    if(str_detect(character_id, pattern = 'HARDNESS')){
      og_sv <- repo$lookups$caco3 %>% 
        mutate(primary = ifelse(str_detect(bioregion, pattern = 'MISS'), 'MVIR', bioregion)) %>% 
        mutate(primary = factor(primary, levels = c("MVIR", "PENNYROYAL", "BLUEGRASS", "MOUNTAINS"))) %>% 
        filter(characteristic %in% 'hardness') %>% 
        select(primary, 'old_sv' = upper_sv)
    }
    if(str_detect(character_id, pattern = 'Alkalinity')){
      og_sv <- repo$lookups$caco3 %>% 
        mutate(primary = ifelse(str_detect(bioregion, pattern = 'MISS'), 'MVIR', bioregion)) %>% 
        mutate(primary = factor(primary, levels = c("MVIR", "PENNYROYAL", "BLUEGRASS", "MOUNTAINS"))) %>% 
        filter(characteristic %in% 'alkalinity') %>% 
        select(primary, 'old_sv' = upper_sv)
    }
    ...
    # plot_path <- 'V:\\DOWWQB\\Assessment\\CALM\\Screening-Values_2023\\summaries\\no_constraintss\\updated_04-24-2024\\index\\'
    render_sv_plots(x = x_list, region = region_def, characteristic = character_id, axis_label = axis_id, trend_plots = FALSE, plot = FALSE)
    file_label <- str_replace(character_id, pattern = ',', replacement = ' ')
    excel_wrapper(out_list)
  # excel_wrapper(out_list, set_width = 20, file_path = 'V:\\DOWWQB\\Assessment\\CALM\\Screening-Values_2023\\summarizes\\no_constraints\\updated_04-24-2024\\index\\', file_id = paste0(file_label, '-updated'))
}
